...
__constant__ unsigned int s[4][4] = {
...
__constant__ unsigned int M[4] = {1, 5, 3, 7};
__constant__ unsigned int T[4] = {0, 1, 5, 0};
...
__device__ void calculateWord_hard(char *placeholder, long long index)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        while (pow(62, len - i - 1) < index)
        {

            if (placeholder[i] == 'Z')
                placeholder[i] = '0';

            else if (placeholder[i] == 'z')
                placeholder[i] = 'A';

            else
                placeholder[i] = (char)((int)placeholder[i] + 1);

            index -= pow(62, len - 1 - i);
        }
    }
...
__global__ void hard_parallel_hard(char *hash_digest, char *return_val, bool *match)
{
    ...
    long long beginIndex = (blockIdx.x * 256000) + (threadIdx.x * 500);

    /* Stores the beginning char array */

    int beginLen = calcWordLength_hard(beginIndex);
    char *begin = new char[beginLen + 1];

    calculateWord_hard(begin, beginIndex);
    ...
    while (count <= 500)
    {
        ...
        hash_gen_device_hard(begin, hash);
