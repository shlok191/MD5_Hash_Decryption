...
__global__ void medium_parallel_medium(char *hash_digest, char *return_val, bool *match)
{
    ...
    int beginLen = calcWordLength_medium(beginIndex);
    char *begin = new char[beginLen + 1];

    calculateWord_medium(begin, beginIndex);
    ...
    while (count <= 500)
    {
        ...
        hash_gen_device_medium(begin, hash);
